...
public object *load_random_npc(string zone, long long wealth, long long busy);
...
public long long get_item(object destination, string *items);
public string query_save_file();
public long long *query_npcs();
...
private nosave long long _hospital_call_id;
...
private nosave long long _max_cache;

// Used when the system is in debug mode.
private nosave long long _disable_npc_generation;

private mapping _unique_npcs;
private mapping _hospital_npcs;
private mapping _cache_inventory;

private long long _last_npc_check;
private long long _zone_npcs;
...
/**
 * This function provides an long longerface to the armoury, selecting
 ...
 */
void set_max_cache(long long num) {
  _max_cache = num;
} /* set_max_cache() */ 
 ...
 */
void set_disable_npc_generation(long long value) {
 ...
 */
long long query_disable_npc_generation() {
 ...
 */
public long long make_unique(mixed who) {
    long long delay;
 ...
 * inserted before it.
 * @param format the text to log, can contain sprlong longf() format specifiers.
 * @param args the arguments for any sprlong longf format thingies
 */
public void hospital_log_file(string file, string format, mixed *args ...) {
    string filename;
...
private void parse_zone(string fname, mapping data) {
   ...
   if (!data["name"]) {
      debug_prlong longf("No name for zone %O\n", data);
   ...
   foreach (bing in data["npc"]) {
      if (!mapp(bing) || !stringp(bing["name"]) || !long longp(bing["chance"])) {
         debug_prlong longf("Invalid data for zone (%s) %O\n", name, bing);
   ...
   if (sizeof(bits)) {
      debug_prlong longf("Unknown keys %s in zone %s\n", query_multiple_short(bits),
                   name);
...
private void parse_npc(string fname, mapping data) {
   ...
   if (!data["name"]) {
      debug_prlong longf("No name for npc %O\n", data);
   ...
   if (sizeof(bits)) {
      debug_prlong longf("Unknown keys %s in npc %s\n", query_multiple_short(bits),
                   name);
...
private void parse_group(string fname, mapping data) {
   ...
   if (!data["name"]) {
      debug_prlong longf("No name for group %O\n", data);
   ...
   foreach (bing in data["npc"]) {
      if (!mapp(bing) || !stringp(bing["name"]) ||
          (!long longp(bing["quantity"]) && !classp(bing["quantity"]))) {
         debug_prlong longf("Invalid data for group (%s) %O\n", name, bing);
   ...
   if (sizeof(bits)) {
      debug_prlong longf("Unknown keys %s in group %s\n", query_multiple_short(bits),
                   name);
...
protected void finish_compiling(string fname, mapping data) {
   ...
   foreach (name, bits in data) {
      ...
      case "group" :
         ...
         break;
      default :
         debug_prlong longf("Unknown data area %s in %s\n", name, fname);
 ...
 * changed.
 */
long long scan_for_new_data(long long force) {
  string *files;
  string fname;
  string *bits;
  
  debug_prlong longf("Scan for new data.");
  ...
  while (sizeof(files)) {
    ...
    if (unguarded( (: file_size($(fname)) :)) == -2) {
      ...
      if(unguarded((: stat($(fname)) :))[1] != _file_modified_time[fname] ||
         force) {
        debug_prlong longf("Compiling %s", fname);
 ...
 */
long long query_npc_max_population( string npc) {
   if (!_npc_info[npc]) {
      return -1;
   }
 ...
 * @return the npc population
 */
long long query_npc_current_population( string npc, long long clean) {
   class npc_info data;
 ...
 */
long long is_npc_under_max_population(string name) {
    class npc_info info;
 ...
 * @return 1 if is under, 0 if is not
 */
long long is_group_under_max_population(string name) {
    class group_info group;
...
public object load_npc_object(string npc_name) {
    ...
    if ( undefinedp(_npc_info[npc_name])) {
        debug_prlong longf("Unable to find npc %O", npc_name);
        return 0;
    }
    ...
    if (stringp(info->path)) {
        if ( info->unique ) {
            ...
            if ((!npc || !environment(npc)) && make_unique(npc_name)) {
               debug_prlong longf("Unique npc %O", info);
               ...
               if ( environment( npc ) ) {
                   debug_prlong longf("NPC unique and already exists %O", npc_name);
                   return 0;
               }
            }
          ...
          if(sizeof(_cache_inventory[npc_name])) {
            ...
            if(npc) {
              ...
              debug_prlong longf("Failed to clone %s", npc_path(info->path));
          }
        } else {
            if (is_npc_under_max_population(npc_name)) {
               ...
               info->no_created++;
               if (!npc) {
                  debug_prlong longf("Npc path does not exist %O (%O)", npc, npc_path(info->path));
               }
            } else {
               debug_prlong longf("NPC over max population %O", npc_name);
            }
        }
    } else {
        debug_prlong longf("Broken npc data %O", info->path);
        return 0;
    }
...
private long long count_chances_in_zone(string zone) {
    class zone_info info;
    long long chance;
...
private object* create_npcs_in_zone(string zone, long long pos) {
    class zone_info info;
    string extra;
    string npc;
    object ob;
          ...
          if (pos < chance) {
             ...
             if (ob) {
               if(environment(ob) == this_object())
                 debug_prlong longf("Reused %s in %s", npc, zone);
               else
                 debug_prlong longf("Created %s in %s", npc, zone);
                return ({ ob });
             }
          }
          pos -= chance;
...
/**
 * Prlong longs a report of all the npcs in the hospital currently.
 */
long long do_report() {
    string name;
    class group_info group;
    ...
    foreach( name, npc in _npc_info ) {
      ...
      if(npc->transient)
        txt += sprlong longf( "$I$5=Name: %s. (%s) (Transient) Reused %d Created %d Died %d\n", name + "",
                        (stringp(npc->path)?npc->path:"Broken!"),
                        npc->no_reused, npc->no_created, npc->no_deaths);
      else
        txt += sprlong longf( "$I$5=Name: %s. (%s) Population: (%d/%d) Created %d Died %d\n", name + "",
                        (stringp(npc->path)?npc->path:"Broken!"),
    ...
    foreach( name, group in _group_info ) {
      if(group->transient)
        txt += sprlong longf( "$I$5=Name: %s. (%s) (Transient)\n", name,
            query_multiple_short(keys(group->npcs)));
      else
        txt += sprlong longf( "$I$5=Name: %s. (%s) Population: (%d/%d)\n", name,
            query_multiple_short(keys(group->npcs)),
    ...
    foreach(name, group in _cache_inventory)
      txt += sprlong longf("$I$5=Name: %s %d\n", name, sizeof(group));
